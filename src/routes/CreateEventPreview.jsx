...
const CreateEventPreview = ({ event, setIsPreview, settings }) => {
  ...
  const handleNavigateBack = () => {
    // Close preview first
    setIsPreview(false);
    // If there is a previous internal page, navigate back
    const isInternal =
      previousPage &&
      previousPage.startsWith("/") &&
      !previousPage.startsWith("//");
    if (isInternal) navigate(-1);
  };
  ...
  // share preview is false for preview mode
  const isShared = false;

  const { handleShare } = useShareEvent(event);

  // Attendance confirmation (preview mode uses the hook but it may be noop)
  const { isPending, handleRespond } = useConfirmAttendance({ event });
              ...
              <div className="flex-wrap items-center gap-1 mb-4 sm:mb-6 mt-2 flex">
                ...
                {event.location &&
                  (event.location.state || event.location.city) && (
                    <TagButton
                      size="lg"
                      text={event.location.state || event.location.city}
                ...
                {/* Chip in type */}
                {chipInDetails && chipInDetails.amount && (
                  <TagButton
                    size="lg"
                    text={`${chipInDetails.chipInType === "fixed" ? "Fixed" : chipInDetails.chipInType === "target" ? "Target" : "From"} - ${chipInDetails.amount}`}
                    variant="light-purple"
                    className="satoshi bold min-w-0 pointer-events-none px-2"
